      ...
      <div v-if="reviewStore.paperAssignments.length">
        ...
        <v-list density="compact" rounded="lg" border class="mb-4">
          <v-list-item
            ...
            :key="a.id"
            :subtitle="assignStatusLabel(a.status)"
...
watch(() => props.paper, async (p) => {
  ...
  try {
    const { data } = await usersApi.reviewersByEvent(auth.eventId);
...
const changeStatus = async () => {
  ...
  try {
    await papersStore.updateStatus(props.paper.id, newStatus.value);
    emit('updated');
